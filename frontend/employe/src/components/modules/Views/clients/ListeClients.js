...
const ListeClients = () => {
    ...
    const filteredCategories = clients
    ...
    // Render rows
    const renderRows = currentRows.map((client) => (
        <tr key={client.id}>
            <td>{client.id}</td>
            <td>{client.name}</td>
            <td>{client.prenom}</td>
            ...
            <td>{client.ribbancaire}</td>
            <td>
                <Link to={`/Historique/${client.id}`}>
            ...
            <td>
                <button style={{ color: "red" }} type="submit" className="btn btn-outline-light" onClick={() => deleteclient(client.id)}>
                    <Delete />
                </button>
            </td>
        </tr>
