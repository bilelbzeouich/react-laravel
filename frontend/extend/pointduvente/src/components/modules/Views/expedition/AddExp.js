...
const AddExp = () => {
    ...
    useEffect(() => {
        axios.get(Constants.BASE_URL + `/clientpvs/${values}`)
            .then(response => {
    ...
    useEffect(() => {
        axios.get(Constants.BASE_URL + `/productpv/${values}`)
            .then(response => {
                console.log(response.data);
                const { data } = response;
                const productArray = Array.isArray(data) ? data : [data]; // Wrap object in an array if it's not already an array
        ...
        setRows((prevRows) => {
            ...
            if (name === 'selectedproduct') {
                ...
                if (name === 'selectedclient') {
                    ...
                    updatedRows[index].pv_id = values;
                }
            ...
            try {
                ...
                setRows((prevRows) => {
                    ...
                    updatedRows[index] = {
                        ...
                        prix: response.data.emplacementName,
                    };
                    return updatedRows;
                });
            } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(Constants.BASE_URL + '/facturation', { rows });
            console.log(response.data);
            setRows((prevRows) =>
                prevRows.map((row) => ({
                    client: '',
